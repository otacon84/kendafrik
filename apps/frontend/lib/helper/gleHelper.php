<?php
  ...
  function myDate($time)
    ...
    $minute = 60;
		...
		{
			$date_mois = intval($since_created / $month).__(' mois');
		}
		else if ($since_created >= $day)
		{
			$date = array(intval($since_created / $day), __('jour'), '');
		}
		else if ($since_created >= $hour)
		{
			$date = array(intval($since_created / $hour), __('heure'), '');
		}
		else if ($since_created >= $minute)
		{
			$date = array(intval($since_created / $minute), __('minute'), '');
		}
		else
		{
			$date = array(intval($since_created), __('seconde'), __('environ'));
		}
